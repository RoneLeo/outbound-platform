	...
	<view class="page">
		...
		<view class="page-block index-head">
			...
			<view class="search-block">
				...
				<input placeholder="搜索任务" maxlength="10" class="search-text" confirm-type="完成" v-model="searchInput" @confirm="searchMe" />
			</view>
		</view>
	...
	export default {
		...
		onLoad() {
			...
			this.$api.post('/dict/findDictListByZddm', {
				...
				this.searchMe(this.currentPage, this.pageSize)
			}).catch((err) => {
		...
		methods: {
			searchMe(page, pagesize) {
				this.isEnd = true; 
				this.isNull = false;
				
				if(typeof pagesize == 'undefined') {  //通过搜索框进来
					page = this.currentPage = 1;
					pagesize = this.pageSize;
				}
				
				this.$api.post('/task/findAllByYwyqyAndCondition', {
					page: page,
					pagesize: pagesize,
					ywyid: this.user.id,
					param: this.searchInput
				}).then(res => {
					this.data = res.data;
			...
			refresh() {
				...
				this.isNull = false;
				this.$api.post('/task/findAllByYwyqy', {
